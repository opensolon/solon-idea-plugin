    ...
    if (!interested.isEmpty()) {
      new Task.Backgroundable(project, "Reloading slon configuration metadata") {
        ...
        public void run(@NotNull ProgressIndicator indicator) {
          ...
          com.intellij.platform.workspace.storage.EntityStorageSnapshot workspace = WorkspaceModel.getInstance(project).getCurrentSnapshot();
          for (int i = 0; i < interested.size(); i++) {
            ...
            if (svc instanceof ModuleMetadataServiceImpl impl) {
              ...
              List<VirtualFile> metaFiles = newDependencies.parallelStream().flatMap(mdi -> {
                if(mdi instanceof ModuleDependencyItem.Exportable.ModuleDependency){
                  ModuleDependencyItem.Exportable.ModuleDependency md = (ModuleDependencyItem.Exportable.ModuleDependency) mdi;
                  return Optional.ofNullable(md.getModule().resolve(workspace)).map(me ->
                          ...
                          .orElseGet(Stream::of);
                }else if (mdi instanceof ModuleDependencyItem.Exportable.LibraryDependency){
                  ModuleDependencyItem.Exportable.LibraryDependency ld = (ModuleDependencyItem.Exportable.LibraryDependency) mdi;
